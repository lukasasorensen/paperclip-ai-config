...
export async function publishToken(api, target, token) {
  ...
  if (!secret || secret.companyId !== target.companyId || secret.name !== 'GH_TOKEN'
      || secret.provider !== 'local_encrypted' || secret.status !== 'active') {
    throw new Error('Rotation target must be an active local_encrypted GH_TOKEN secret in the configured company.');
  }
